...
# Initiate the models lazily so the server can start even if the API key is missing.
...
def _delete_existing_for_website(website_url: str) -> None:
    ...
    try:
        get_vector_store()._collection.delete(where={"website_url": website_url})
    except Exception:
...
def ingest_website(url: str, max_depth: int = 2):
    ...
    try:
        ...
        for i in range(0, len(chunks), EMBED_BATCH_SIZE):
            ...
            get_vector_store().add_documents(documents=batch, ids=uuids)
            added += len(batch)
